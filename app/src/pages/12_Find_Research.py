...
name_result = requests.get(f'http://api:4000/d/departments/d{department_id}').json()
department_name = name_result[0].get('Name', 'No Name')

# set the header of the page
st.header(f'Research Opportunities in {department_name} Department')

# make an API request to fetch posts based on selected group
response = requests.get(f'http://api:4000/o/opportunities/d{department_id}')
...
if response.status_code == 200:
    ...
    for opportunity in response:
        ...
        proffessor_email = response2[0].get('Email')
        ...
        st.markdown(f"""
            <div style="border: 1px solid #3D4A59; padding: 20px; margin-bottom: 20px; border-radius: 8px; box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.2); background-color: #3D4A59;">
                ...
                <p style="margin-bottom: 8px; color: #D1D7DC;"><strong>Professor Email:</strong> {proffessor_email}</p>
